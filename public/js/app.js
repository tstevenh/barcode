(function () {
  ...
  // ---- Sidebar ----
  const RESERVED_SLUGS = new Set(["barcode", "barcodes", "api", "api-docs", "health", "index", "css", "js", "sitemap", "robots"]);
  function slugifyType(name) {
    return String(name).toLowerCase().replace(/\+/g, " plus ").replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "");
  }
  function typeSlugMap() {
    const map = {};
    const seen = new Set();
    CATALOG.forEach((grp) => grp.items.forEach((it) => {
      let slug = slugifyType(it.name);
      if (RESERVED_SLUGS.has(slug)) slug += "-barcode";
      while (seen.has(slug)) slug += "-" + slugifyType(it.id);
      seen.add(slug);
      map[it.id] = slug;
    }));
    return map;
  }
  const TYPE_SLUGS = typeSlugMap();
  function currentUrlLocalePrefix() {
    const seoLangs = ["pl", "de", "nl", "fr", "ja"];
    const first = (window.location.pathname || "/").split("/").filter(Boolean)[0];
    return seoLangs.includes(first) ? "/" + first : "";
  }
  function typePageHref(id) {
    return `${currentUrlLocalePrefix()}/${TYPE_SLUGS[id] || slugifyType(id)}`;
  }
  function buildSidebar(filter) {
    ...
    CATALOG.forEach((grp) => {
      ...
      matches.forEach((it) => {
        ...
        b.textContent = it.name; b.dataset.id = it.id; b.href = typePageHref(it.id);
        b.addEventListener("click", (event) => {
          if (event.metaKey || event.ctrlKey || event.shiftKey || event.altKey || event.button) return;
          if (window.location.pathname.replace(/\/$/, "") === new URL(b.href, window.location.href).pathname.replace(/\/$/, "")) {
  ...
  function localizedUrlFor(nextLang) {
    ...
    if (/^\/api-docs(?:\.html)?\/?$/.test(path)) return "";
    const parts = path.split("/").filter(Boolean);
    if (parts.length && seoLangs.includes(parts[0])) parts.shift();
    let leaf = parts.join("/");
    if (!leaf || leaf === "index.html") leaf = "";
    if (leaf.endsWith(".html")) leaf = leaf.slice(0, -5);
    const prefix = nextLang === "en" ? "" : "/" + nextLang;
    const nextPath = leaf ? `${prefix}/${leaf}` : `${prefix || "/"}`;
